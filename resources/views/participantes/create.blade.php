<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">{{ __('Crear participante') }}</h2>
    </x-slot>
            ...
            <div class="bg-white shadow-sm sm:rounded-lg border border-slate-200 p-6">
                <form method="POST" action="{{ route('participantes.store') }}" class="space-y-6">
                    ...
                    <div class="flex gap-3">
                        ...
                        <a href="{{ route('participantes.index') }}" class="rounded-md border border-slate-300 px-4 py-2 text-sm text-slate-700">Cancelar</a>
                    </div>
                </form>
            </div>
        </div>
